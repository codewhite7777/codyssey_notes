...
> **TLDR** · iptables/nftables는 커널 layer, ufw·firewalld는 그 위의 사용자 인터페이스. 기본 거부 + 명시적 허용이 원칙. **SSH 포트를 활성화 전에 허용 안 하면 본인 세션이 즉시 끊김** — 가장 흔한 사고이자 클라우드에서는 콘솔로 가야 복구 가능.
...
flowchart TD
    A[사용자 명령] --> B{어떤 도구?}
    B -->|ufw allow 22| C[ufw<br/>Ubuntu·Debian 표준<br/>단순 인터페이스]
    B -->|firewall-cmd ...| D[firewalld<br/>RHEL·Fedora 표준<br/>zone 기반]

    C --> E[iptables 또는 nftables<br/>규칙 관리 명령]
    D --> E

    E --> F[netfilter<br/>커널 프레임워크]
    F --> G[패킷 검사·필터링·변환]
...
> [!WARNING]
> **활성화 전 SSH 포트 허용 필수**: `ufw enable` 직전에 22(또는 변경된 포트)를 allow 안 하면 즉시 본인 세션 끊김 → 클라우드 콘솔로 복구해야 함. 안전 패턴: (1) SSH 포트 먼저 허용 → (2) 다른 터미널에 새 세션 → (3) enable → (4) 새 세션 살아있는지 확인.
...
<details>
<summary><b>응용 토픽 — nftables·eBPF 방화벽·namespace·클라우드 layered (펼치기)</b></summary>
