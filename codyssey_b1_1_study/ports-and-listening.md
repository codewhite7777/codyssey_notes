...
> **TLDR** · 포트는 16비트 식별자로 한 호스트의 여러 서비스를 다중화한다. LISTEN 상태는 서비스 가용성의 1차 신호로, 이 상태가 없으면 들어오는 연결은 모두 RST로 거부됨. `0.0.0.0`(모든 인터페이스) vs `127.0.0.1`(로컬만)의 구분이 외부 노출의 기준.
...
stateDiagram-v2
    ...
    CLOSED --> LISTEN: 서버 listen()
    LISTEN --> SYN_RECV: SYN 수신
    SYN_RECV --> ESTABLISHED: ACK 수신

    CLOSED --> SYN_SENT: 클라이언트 connect()
    SYN_SENT --> ESTABLISHED: SYN+ACK 수신

    ESTABLISHED --> FIN_WAIT_1: 자기 close()
    ESTABLISHED --> CLOSE_WAIT: FIN 수신

    FIN_WAIT_1 --> FIN_WAIT_2: ACK 수신
    FIN_WAIT_2 --> TIME_WAIT: FIN 수신
    TIME_WAIT --> CLOSED: 2MSL 대기 ★

    CLOSE_WAIT --> LAST_ACK: close()
    LAST_ACK --> CLOSED: ACK 수신

    note left of LISTEN: ★ 서비스 가용성의<br/>1차 신호
    note right of TIME_WAIT: ★ 누적 시<br/>ephemeral 고갈
```
...
> [!WARNING]
> **부하 시 빈번한 사고**: 짧은 연결을 대량 만드는 패턴(매 요청마다 새 연결)에서 TIME_WAIT가 누적되어 ephemeral 포트가 고갈 → 새 연결 거부. `net.ipv4.tcp_tw_reuse=1` sysctl로 임시 완화 가능하지만 근본적으로는 connection pooling으로 해결.
...
<details>
<summary><b>응용 토픽 — epoll·소켓 옵션·eBPF·namespace·클라우드 SG (펼치기)</b></summary>
