...
> **TLDR** · 12비트 mode = 특수 3비트(setuid/setgid/sticky) + 9비트(owner/group/other × rwx). 권한 검사는 owner > group > other 순으로 **첫 매칭만 적용**하므로 owner를 좁게 두면 group이 있어도 무력. 디렉토리에서 `x`는 'traverse 권한'으로 `r`과 독립적이다.
...
```mermaid
flowchart TD
    A[프로세스가 파일에 접근 시도] --> B{root이고<br/>CAP_DAC_OVERRIDE?}
    B -->|Yes| Z1[✓ 허용]
    B -->|No| C{호출자 uid ==<br/>file owner uid?}
    C -->|Yes| D[owner 권한 비트 검사]
    D -->|매칭| Z1
    D -->|거부| Z2[✗ EACCES<br/>★ group 검사 안 함]
    C -->|No| E{호출자 gid ==<br/>file group OR<br/>supplementary?}
    E -->|Yes| F[group 권한 비트 검사]
    F -->|매칭| Z1
    F -->|거부| Z2
    E -->|No| G[other 권한 비트 검사]
    G -->|매칭| Z1
    G -->|거부| Z2
...
> [!WARNING]
> `chmod 777`은 거의 항상 잘못된 답이다. 누구나 수정·삭제 가능하게 만들어 보안 사고로 이어짐. 권한 문제는 항상 **정확한 설정**으로만 해결할 것. 재귀 변경 시에도 8진법(`-R 755`) 대신 심볼릭+대문자 X(`-R u+rwX,g+rX,o-rwx`) 사용.
...
<details>
<summary><b>응용 토픽 — ACL·MAC·capabilities·immutable·mount options (펼치기)</b></summary>
