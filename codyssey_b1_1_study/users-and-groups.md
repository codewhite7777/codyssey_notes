...
> **TLDR** · Linux 신원 = `(uid, gid, supplementary GIDs)`. real/effective/saved 3종 UID 분리가 setuid·sudo의 메커니즘. 그룹은 primary 1개 + supplementary 0개 이상이며, `usermod -aG`로 추가해야 안전 — `-G`만 쓰면 기존 그룹이 다 날아간다.
...
세 가지 UID(real, effective, saved)가 분리되어 있는 이유는 특권 프로그램의 권한 위임 메커니즘 때문이다. `passwd` 명령을 일반 사용자가 실행하는 경우를 생각해보자.

```mermaid
sequenceDiagram
    participant U as 일반 사용자<br/>(uid=1001)
    participant P as passwd<br/>(setuid 비트)
    participant K as 커널
    participant S as /etc/shadow<br/>(root만 write 가능)

    U->>P: passwd 실행
    Note over P: 파일 권한: -rwsr-xr-x<br/>★ setuid 비트 set
    K->>K: effective UID = 0 (root)로 변경
    Note over P: real UID: 1001 유지 (감사 추적용)<br/>effective UID: 0 (권한 행사용)
    P->>S: 비밀번호 해시 쓰기
    S-->>P: ✓ 쓰기 성공
    P-->>U: 완료
    Note over K: 종료 시 effective UID 1001로 복귀
```

이런 분리 구조는 sudo, su, screen lock, 일부 데몬의 권한 관리에 모두 활용되며, 단순해 보이는 "사용자 신원"이 실제로는 권한 동작을 정밀하게 통제하기 위한 다층 구조라는 점을 드러낸다. real UID는 추적성을, effective UID는 실제 권한을, saved UID는 권한 임시 drop/restore를 책임진다.
...
> [!WARNING]
> **가장 흔한 사고**: `usermod -G newgroup user`는 supplementary group을 **통째로 교체**. 기존 모든 그룹이 한 번에 사라진다. 추가하려면 반드시 `-aG` (append) 플래그.
...
<details>
<summary><b>응용 토픽 — capabilities·user namespace·PAM·systemd-homed·ACL (펼치기)</b></summary>
