...
> **TLDR** · `/etc/ssh/sshd_config` 한 파일로 SSH 보안 전체가 결정. 같은 옵션 중복 시 **첫 줄만 적용**(직관과 반대). 변경 후 `systemctl reload sshd` 필수 + 잘못된 설정으로 lockout 위험 → 새 세션을 다른 터미널에 미리 열어두는 게 안전 패턴.
...
안전한 운영 워크플로우는 다음 순서를 따른다 — 특히 원격 서버에서 작업할 때 lockout을 방지하는 핵심 패턴이다.

```mermaid
flowchart TD
    A[1. sshd_config 편집] --> B[2. sshd -t<br/>문법 검증]
    B -->|에러| X[수정 후 재시도]
    X --> B
    B -->|OK| C[★ 3. 다른 터미널에서<br/>새 SSH 세션 미리 열기]
    C --> D[4. systemctl reload sshd]
    D --> E[5. sshd -T로<br/>효과적 설정 확인]
    E --> F[6. 새 SSH 접속 테스트]
    F -->|성공| G[7. 첫 세션 종료 OK]
    F -->|실패| H[★ 미리 열어둔 세션에서<br/>롤백 후 재시도]
    H --> A
...
> [!WARNING]
> **가장 위험한 사고**: 원격 서버에서 SSH 설정 변경 후 재시작 시 본인 접속이 끊김. 클라우드 인스턴스면 콘솔로 가야 복구 가능. **변경 전 다른 터미널에서 새 SSH 세션을 미리 열어두는** 안전 패턴 필수.
...
<details>
<summary><b>응용 토픽 — fail2ban·jump host·SSH 인증서·SFTP (펼치기)</b></summary>
