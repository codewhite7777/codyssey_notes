...
권한 모델은 단순해 보이지만 운영에서 여러 미묘한 함정이 있다.

**`chmod 777`은 거의 항상 잘못된 답이다**. 권한 문제를 해결하려고 임시로 777로 풀어두면, 어떤 사용자든 그 파일·디렉토리를 수정·삭제할 수 있게 된다. 디렉토리에 777을 주면 누구나 임의 파일을 생성할 수 있어서 백도어 설치 경로가 된다. "권한 문제는 정확한 권한 설정으로만 해결한다"가 원칙이다.

**재귀 chmod에 숫자를 사용하는 것**도 흔한 실수다. `chmod -R 755 dir/`을 실행하면 디렉토리뿐 아니라 그 안의 데이터 파일에도 x 비트가 부여된다. 데이터 파일에 x는 의미 없을 뿐 아니라, 만약 그 파일이 우연히 ELF 헤더로 시작하면 실행 가능 바이너리로 인식되어 보안 도구가 경고를 내거나 의도하지 않은 동작을 할 수 있다. 재귀 변경에는 심볼릭 표기와 대문자 X를 사용해야 한다.

**파일 삭제 권한이 부모 디렉토리에 있다**는 점도 자주 함정이 된다. read-only로 보이는 파일을 사용자가 삭제할 수 있다면 그것은 부모 디렉토리에 write 권한이 있기 때문이다. sticky 비트(`chmod +t dir/`)로 방어할 수 있지만, 이 메커니즘 자체를 모르면 의도하지 않은 삭제가 일어난다.

**setuid 셸 스크립트는 거의 모든 OS가 무시한다**. 셸 스크립트에 setuid 비트를 설정해도 실행 시점에 race condition으로 다른 스크립트로 바꿔치기될 수 있는 보안 이슈 때문이다. setuid가 진짜 필요하면 컴파일된 wrapper를 만들고 그 안에서 스크립트를 호출해야 한다. 또는 sudo 정책으로 대체하는 것이 일반적이다.

**owner가 group보다 권한이 적으면 owner는 그 권한을 못 받는다**. 권한 검사 알고리즘이 owner 매칭을 먼저 시도하고 그 결과를 적용하기 때문이다. 의도적으로 이런 설정을 하는 경우는 드물지만, 자동화 스크립트가 권한을 잘못 계산하면 만들 수 있다.

**NFS의 권한 함정**은 분산 환경에서 만난다. NFS 클라이언트와 서버가 다른 사용자 매핑을 가지면 같은 uid가 다른 사용자를 가리킬 수 있다. `no_root_squash` 옵션을 켠 채 NFS export하면 클라이언트의 root가 서버에서도 root로 작용해 보안 모델이 깨진다.

**umask의 cron 함정**은 운영 자동화에서 자주 만난다. cron이 만드는 파일은 cron 데몬에서 상속한 umask 022로 생성되므로 group/other에게 read 가능하다. 보안 민감 데이터를 만드는 cron 스크립트는 첫 줄에서 명시적으로 umask를 강하게 설정해야 한다.

**심볼릭 링크의 권한**은 보통 무시된다. `ls -l symlink`로 보면 항상 `lrwxrwxrwx`로 보이지만 이 권한은 의미 없다 — 실제 권한 검사는 링크가 가리키는 대상에 적용된다. `chmod`로 심볼릭 링크의 권한을 바꾸려고 하면 대상이 바뀌고 링크 자체는 그대로다(`-h` 옵션 예외).
...
권한 모델의 확장 학습으로 다음 토픽들이 가치 있다.

**POSIX ACL**은 9비트로 표현 불가능한 권한 요구를 다루는 확장이다. `setfacl`, `getfacl` 명령으로 사용자·그룹별 세밀한 권한을 부여할 수 있다. ext4, XFS 등 대부분의 현대 파일시스템이 지원한다.

**MAC (Mandatory Access Control)**은 SELinux나 AppArmor로 구현되며, DAC 위에 추가되는 강제 정책 layer다. DAC에서는 root가 모든 것을 할 수 있지만, MAC은 root조차 정책을 우회할 수 없게 만든다. 컨테이너 보안, 데몬 격리에 자주 활용된다.

**capabilities**는 root 권한을 잘게 쪼갠 메커니즘이다. setuid의 안전한 대안으로, 예를 들어 ping이 root setuid 없이 raw 소켓을 사용할 수 있도록 `cap_net_raw`만 부여하는 식이다.

**immutable 비트**(`chattr +i file`)는 root조차 수정·삭제할 수 없게 만드는 확장 속성이다. 시스템 무결성 보호에 사용되며, 일반 chmod로는 보이지 않으므로 함정이 되기도 한다.

**mount option으로의 권한 제한**은 파티션 단위 보안이다. `noexec`로 그 파티션에서 실행 파일을 못 돌리게, `nosuid`로 setuid 비트를 무시하게, `nodev`로 디바이스 노드를 못 만들게 할 수 있다. 일반적으로 `/tmp`나 외부 마운트에 적용한다.
