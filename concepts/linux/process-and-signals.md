...
**`ps`의 %CPU는 누적 평균**이라는 점이 자주 오해된다. 시작 후 (CPU 사용 시간 / 경과 시간)이라서, 오래 살아있는 프로세스는 실제로 지금은 idle해도 평균이 높을 수 있다. 순간 부하를 보려면 `top` 또는 `pidstat 1`을 쓴다.

**PID 재사용**은 자동화 스크립트의 미묘한 함정이다. exit한 프로세스의 PID는 곧 재사용된다. 오래된 PID를 캐시했다가 그걸로 `kill`을 보내면, 의도한 프로세스가 아니라 같은 PID를 받은 다른 프로세스를 죽일 수 있다. `pgrep -f`로 명령줄까지 함께 검증하는 게 안전하다.

**`kill -9`로 인한 데이터 손상**은 운영 사고의 단골이다. DB 트랜잭션 중간이거나 파일 write 중간에 SIGKILL을 받으면 일관성이 깨진 상태로 남는다. SIGTERM → 대기 → SIGKILL 패턴을 항상 준수해야 한다.

**D 상태 프로세스에 SIGKILL 안 통함**은 위에서 설명한 대로다. NFS 끊김이나 dying disk가 흔한 원인이다. 재부팅 외에 해결 방법이 없는 경우가 많다.

**시그널 핸들러 안에서 안전하지 않은 함수 호출**은 시그널 프로그래밍의 미묘한 함정이다. 시그널 핸들러는 main 흐름을 인터럽트하므로, 비-reentrant 함수(`printf`, `malloc` 등)를 호출하면 정의되지 않은 동작이 일어난다. `man 7 signal-safety`에 사용 가능한 함수 목록이 있는데, 매우 짧다.

**fork bomb**은 보안 사고로 이어질 수 있다. `:(){ :|:& };:` 같은 재귀 fork는 시스템의 PID 공간과 메모리를 빠르게 고갈시킨다. cgroup의 `pids.max`로 PID 수를 제한해 방어한다.

**wait()의 race**는 시그널 프로그래밍에서 자주 만난다. `SIGCHLD` 핸들러에서 `waitpid(-1, ...)`를 한 번 호출하면 한 자식만 reap한다. 한 번에 여러 자식이 죽으면 SIGCHLD가 합쳐져서 한 번만 전달될 수 있고, 그러면 reap 안 된 자식이 좀비로 남는다. 핸들러 안에서 loop으로 모든 자식을 reap해야 안전하다.

**`top`의 %CPU가 100%를 초과**하는 것은 멀티코어 환경에서 정상이다. 단일 프로세스가 N개 코어를 점유하면 N×100%까지 가능하다. `Shift+I`로 Irix mode를 토글하면 단일 코어 기준 백분율로 전환된다.

**double fork 패턴**은 데몬화의 전통적 패턴이다. 첫 번째 fork로 자식을 만들고, 자식이 다시 fork해서 손자를 만들고 자식은 죽으면, 손자는 init에 입양된다. 이렇게 하면 손자가 controlling terminal에서 분리되고 PID 1을 부모로 갖게 된다. systemd의 `Type=forking`이 이 패턴을 처리한다.

**SIGPIPE 무시**는 네트워크 서버에서 중요하다. 파이프라인의 receiver가 죽으면 sender가 SIGPIPE로 종료된다. 네트워크 서버는 보통 `SIG_IGN`으로 SIGPIPE를 무시하고 EPIPE 에러를 직접 처리한다 — 그래야 한 클라이언트가 끊겼다고 서버 전체가 죽지 않는다.
...
프로세스 모델의 확장 학습으로 다음 토픽들이 가치 있다.

**Process group, session, controlling terminal**은 잡 컨트롤(job control)의 기반이다. `setpgid`, `setsid` 같은 시스템 콜로 프로세스 그룹을 만들고, 셸이 Ctrl+Z (SIGTSTP)나 Ctrl+C (SIGINT)를 어디로 보낼지 결정한다.

**`prctl(2)`**는 프로세스의 다양한 속성을 세밀하게 제어하는 인터페이스다. `PR_SET_NAME` (프로세스 이름 변경), `PR_SET_PDEATHSIG` (부모가 죽으면 자식에게 시그널), `PR_SET_NO_NEW_PRIVS` (privilege escalation 차단) 등의 기능을 제공한다.

**cgroups (v1, v2)**는 리소스 격리·제한·계측의 핵심 메커니즘이다. CPU, 메모리, IO, PID 등을 그룹 단위로 제한할 수 있다. systemd, Docker, Kubernetes가 모두 cgroups 위에 쌓여 있다. cgroups v2는 v1의 여러 한계를 해결한 새 버전이며 현재 표준이다.

**Namespace**는 프로세스의 view를 격리하는 메커니즘이다. PID namespace, network namespace, mount namespace, user namespace 등이 있다. 컨테이너의 격리는 모두 namespace의 조합으로 만들어진다.

**eBPF + tracing**은 프로세스 동작을 관측하는 현대적 도구다. `bpftrace`, `bcc`, `perf` 같은 도구로 syscall, schedule, cache miss 등을 매우 낮은 오버헤드로 추적할 수 있다. production 디버깅의 게임체인저다.

**systemd**는 service unit으로 프로세스 lifecycle을 관리하는 init system이다. `Type=`, `Restart=`, `KillSignal=`, `TimeoutStopSec=` 같은 옵션으로 graceful shutdown, 재시작 정책, 의존성을 선언적으로 표현한다.

**OOM killer**는 메모리 압박 시 OS가 프로세스를 골라 SIGKILL하는 메커니즘이다. `oom_score`라는 휴리스틱으로 어떤 프로세스를 죽일지 결정하며, `/proc/PID/oom_score_adj`로 가중치를 조정할 수 있다. B1-2 OOM 트러블슈팅에서 자세히 다룬다.
