...
| 모드 | 적용 시점 | 특징 |
|---|---|---|
| **중급자 (기본)** | CS 기초 있음, 시스템/운영 학습 중 | 기술 용어 적극, 동작 원리·설계 의도 깊이, 인접 토픽 연결, 100-200줄 |
| **초심자** | 특정 토픽이 완전히 새로움 | 일관된 비유, 용어 풀이 분리, 다이어그램 다수, ~100줄 |
| **고급/ODIN** | 본인 명시 요청 | 글로벌 ODIN 컨벤션 (압축, VS) |

현재 노트는 **중급자 모드** 기준으로 작성. 컨테이너·cgroups·eBPF 같은 인접 토픽 언급은 깊이 들어가지 않더라도 "있다는 걸 알게" 함.
